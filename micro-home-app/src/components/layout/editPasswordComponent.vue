...
export default{
	...
	data () {
		const isSameCheck = (rule, value, callback) => {
			if(hex_sha1(value) != this.oldPassword){
	...
	methods:{
		initData(flag){
			if(flag){
				this.oldPassword = window.memoryStorage.getItem("USERINFO").loginPassword;
		...
		editPwdSubmit(){
			this.$http.post('sysUser/modifyPwd',{oldPassword:hex_sha1(this.formPassword.oldPassword),newPassword:hex_sha1(this.formPassword.newPassword1)},{emulateJSON: true})
			.then(function(res){				
				if(res.data.code == 0){
					this.$Notice.success({
						desc: "修改成功，请重新登录"
