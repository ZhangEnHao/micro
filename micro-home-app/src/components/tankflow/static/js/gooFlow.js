 ...
 * blurItem				清除选中
 * loadData				载入数据
 * this.$workArea.delegate(".ico","mousedown",{inthis:this},function(e){		移动节点代码
 * this.$tool.on("mousedown",{inthis:this},function(e){							添加节点代码
 */
import {
	mousePosition,
	getElCoordinate,
...
var _this;
//定义一个区域图类：
function GooFlow(bgDiv, property, referenceType, vueThis) {
	scrollBarLeft = 0;
	scrollBarTop = 0;
	svgId; //导出流程图
	newSizeFlag = false;
	...
	_top = bgDiv.offset().top;
	_left = bgDiv[0].offsetLeft;
	this.$bgDiv.addClass("GooFlow");
	if (this.color.font) {
		this.$bgDiv.css("color", this.color.font);
	}
	//	var width=$(window).width()*0.98;//(property.width||800)-2;
	//	var height=$(window).height()*0.96;//(property.height||500)-2;
	var width, height;
	switch (referenceType) {
		case "instance":
			width = this.$bgDiv.width()+2;
			height = this.$bgDiv.height()+2;
			break;
		case "designer":
			//自适应
			width = (property.width || this.$bgDiv.width());
			height = (property.height || this.$bgDiv.height());
			break;
	}
	//var width=this.$bgDiv.width();//(property.width||800)-2;
	//var height=this.$bgDiv.height();//(property.height||500)-2;
	this.$bgDiv.css({
		width: width + "px",
		height: height + "px"
	});
	...
	this.$title = referenceType; //流程图的名称
	//this.$title="instance";//流程图的名称
	...
	this.$nodeDom = {};
	this.$areaDom = {};
	this.$max = property.initNum || 2; //计算默认ID值的起始SEQUENCE
	this.$maxWidth = 0;
	this.$maxHeight = 0;
	...
	this.$scale = 1.0; //工作区内容的缩放比例，从0.1至无穷大，初始默认为1
	// 移动节点时的辅助线
	this.$auxiliaryLine = [];
	...
	if (property.haveHead) {
		tmp = "<div class='GooFlow_head' " + (this.color.main ? "style='border-bottom-color:" +
			this.color.main + "'" : "") + ">";
		if (property.headLabel) {
			tmp += "<label title='" + (property.initLabelText || "newFlow_1") + "' " +
				(this.color.main ? "style='background:" + this.color.main + "'" : "") +
	...
	var toolHeight = $(window).height() * 0.89 - 48;
	var toolWidth = 0;
	if (property.haveTool) {
		this.$bgDiv.append("<div class='GooFlow_tool'" + (property.haveHead ? "" : " style='margin-top:3px'") +
			"><div class='GooFlow_tool_div'></div></div>");
		this.$tool = this.$bgDiv.find(".GooFlow_tool div");
		//未加代码：加入绘图工具按钮
		this.$tool.append("<a href='javascript:void(0)' type='cursor' class='GooFlow_tool_btndown' id='" +
			this.$id + "_btn_cursor'><i class='ico_cursor'/></a>" +
			"<a href='javascript:void(0)' type='direct' class='GooFlow_tool_btn' id='" +
			this.$id + "_btn_direct'><i class='ico_direct'/></a>");

		if (property.toolBtns && property.toolBtns.length > 0) {
			tmp = "<span/>";
			for (var i = 0; i < property.toolBtns.length; ++i) {
				tmp += "<a href='javascript:void(0)' type='" + property.toolBtns[i] +
					"' id='" + this.$id + "_btn_" + property.toolBtns[i].split(" ")[0] +
					"' class='GooFlow_tool_btn'><i class='ico_" + property.toolBtns[i] + "'/></a>"; //加入自定义按钮
			}
			this.$tool.append(tmp);
		}
		//加入区域划分框工具开关按钮
		if (property.haveGroup)
			this.$tool.append("<span/><a href='javascript:void(0)' type='group' class='GooFlow_tool_btn' id='" + this.$id +
				"_btn_group'><i class='ico_group'/></a>");
		switch (this.$title) {
			case "instance":
				toolWidth = 68; //右侧工具栏 宽度设置
				break;
			case "designer":
				toolWidth = 31; //右侧工具栏 宽度设置
				break;
		}
		this.$nowType = "cursor";
		editable = true; //只有具有工具栏时可编辑
	}
	//width = width - toolWidth - 40;
	//	height=height-headHeight-(property.haveHead? 22:25);
	height = toolHeight;
	// 初始化画布div
	switch (this.$title) {
		case "instance":
			this.$bgDiv.append("<div class='GooFlow_work notool' " +
				"style='overflow:auto;" +
				(property.haveHead ? "" : "margin-top:3px") + "'></div>");
			...
			this.initDraw("draw_" + this.$id, width * 0.97, height * 0.95);
			break;
		case "designer":
			this.$bgDiv.append("<div class='GooFlow_work' " +
				"style='overflow:auto;" +
				(property.haveHead ? "" : "margin-top:3px") + ";margin-bottom:3px;margin-left:50px;'></div>");
			this.$workArea = $("<div class='GooFlow_work_inner' id='gundongtiao' " +
			...
			this.$bgDiv.children(".GooFlow_work").append(this.$workArea);
			this.$draw = null; //画矢量线条的容器
			this.initDraw("draw_" + this.$id, (width - 10), (height - 10));
			break;
	}
	this.$group = null;
	if (property.haveGroup)
...
//			debugger
			//This.addNode(This.$id+"_node_"+This.$max,{name:"node_"+This.$max,left:X,top:Y,type:This.$nowType,data:"{}"});//添加data属性 libin修改Id
			//添加流程节点组件，添加data属性 libin修改Id
			// 改为拖动节点的方式增加新节点
			This.addNode(This.$max, 
					{name: "node_" + This.$max, 
					left: (X+document.getElementById("gundongtiao").scrollLeft) / This.$scale,
					top: (Y+document.getElementById("gundongtiao").scrollTop) / This.$scale,
		...
		this.$textArea = $("<textarea style='font-size: 12px;'></textarea>");
		...
		this.$lineMove.on("mousedown", {
			...
			var lm = $(this);
			lm.css({
				"background-color": e.data.inthis.color.font
			});
			var This = e.data.inthis;
			var ev = mousePosition(e),
		...
		//选定一条转换线后出现的浮动操作栏，有改变线的样式和删除线等按钮。//选定线时显示的操作框
		this.$lineOper = $("<div class='GooFlow_line_oper' style='display:none'><i class='b_l1'></i><i class='b_l2'></i>" +
			"<i class='b_l3'></i><i class='b_x'></i></div>");
		...
		if (property.useOperStack && editable) { //如果要使用堆栈记录操作并提供“撤销/重做”的功能,只在编辑状态下有效
			this.$undoStack = [];
			this.$redoStack = [];
			this.$isUndo = 0;
			///////////////以下是构造撤销操作/重做操作的方法
...
GooFlow.prototype = {
	...
	initDraw: function(id, width, height) {
		...
		if (this.useSVG != "") {
			...
			//libin创建defs 添加maker，<line>并没有箭头，可以通过<marker>进行扩展
			var defs = document.createElementNS("http://www.w3.org/2000/svg", "defs");
			this.$draw.appendChild(defs);
			// 未选中状态
			defs.appendChild(this.getSvgMarker("arrow1", this.color.line));
			// 选中
			defs.appendChild(this.getSvgMarker("arrow2", this.color.lineMarked));
			// 执行成功
			defs.appendChild(this.getSvgMarker("arrow2_1", this.color.lineMarked));
			defs.appendChild(this.getSvgMarker("arrow3", this.color.lineMarked));
			defs.appendChild(this.getSvgMarker("arrow3_1", this.color.lineMarked));
			defs.appendChild(this.getSvgMarker("arrow_lineSuccess", this.color.lineSuccess));
		} else {
			...
			this.$workArea.prepend("<div class='GooFlow_work_vml' style='position:relative;'></div>");
		...
		$(this.$draw).delegate(tmpClk, "dblclick", {
			...
			This.$textArea.val(oldTxt).css({
				...
				top: t.top - 10 + y - This.$workArea[0].parentNode.scrollTop
	...
	initGroup: function(width, height) {
		...
		this.$group.on("mousedown", {
			...
			if (cursor != "move") {
				This.$ghost.css({
					display: "block",
					width: This.$areaData[id].width * This.$scale - 2 + "px",
					height: This.$areaData[id].height * This.$scale - 2 + "px",
			...
			document.onmousemove = function(e) {
				...
				if (cursor != "move") {
					...
					else if (X + This.$workArea[0].parentNode.scrollLeft + This.$areaData[id].width * This.$scale > t.left + This
						.$workArea.width())
					...
					else if (Y + This.$workArea[0].parentNode.scrollTop + This.$areaData[id].height * This.$scale > t.top + This.$workArea
						.height())
			...
			document.onmouseup = function(e) {
				...
				if (cursor != "move")
					...
					This.moveArea(id, (X + This.$workArea[0].parentNode.scrollLeft - t.left) / This.$scale, (Y + This.$workArea[0]
						.parentNode.scrollTop - t.top) / This.$scale);
				return false;
			}
		});
		//绑定修改文字说明功能
	...
	initLinePointsChg: function() {
		this.$mpFrom.on("mousedown", {
			...
			var line = this.drawLine("GooFlow_tmp_line", [ps[0], ps[1]], [pe[0], pe[1]], true, true);
	...
	addNode: function(id, json) {
		...
		if (json.type.indexOf(" round") < 0) {
			...
			if (this.$title == "instance") {
				...
				if (typeof(json.state) == 'number' && (json.type == "api" || json.type == "scr" ||
						json.type == "cmd" || json.type == "sub-process")) {
					addi = "<img class='position jiao" + json.state + "'/>";
					//
					if (json.state == 0) { /** 节点正常结束 */
						addi =
							"<div class='done d-success anim'><a href='' target='_blank'><svg xmlns='http://www.w3.org/2000/svg' xmlns:xlink='http://www.w3.org/1999/xlink' version='1.1' x='0px' y='0px' viewBox='0 0 1000 1000'><g><path id='path' d=' M748.4,325L448,623.1L301.6,477.9c-4.4-4.3-11.4-4.3-15.8,0c-4.4,4.3-4.4,11.3,0,15.6l151.2,150c0.5,1.3,1.4,2.6,2.5,3.7c4.4,4.3,11.4,4.3,15.8,0l308.9-306.5c4.4-4.3,4.4-11.3,0-15.6C759.8,320.7,752.7,320.7,748.4,325z' <='' g=''></path></g></svg></a></div>";
					}
					if (json.state == 1 || json.state == 7) { /** 节点异常结束、节点运行超时 */
						addi = "<div class='done d-danger anim'><i class='ivu-icon ivu-icon-md-close'></i></div>";
					}
					if (json.state == 6) { /** 节点运行超时 */
						addi = "<div class='timeout anim'><i class='timeout-warning ivu-icon ivu-icon-ios-timer'></i></div>";
					}
					if (json.state == 2 || json.state == 8) { /** 节点正常暂停、调度节点为未激活状态*/
						addi = "<div class='done d-success anim'><i class='ivu-icon ivu-icon-ios-pause'></i></div>";
					}
					if (json.state == 3) { /** 节点异常暂停状态*/
						addi = "<div class='done d-danger anim'><i class='ivu-icon ivu-icon-ios-pause'></i></div>";
					}
					if (json.state == 4) { /** 节点运行中 */
						addi =
							"<div class='done d-success anim'><i class='ivu-icon ivu-icon-md-refresh demo-spin-icon-load'></i></div>";
					}
			...
			if (newSizeFlag) {
				var newStyle = "width:" + newIcoSize["width"] + ";height:" + newIcoSize["height"] + ";font-size:" + newIcoSize[
						"font-size"] +
				...
				this.$nodeDom[id] = $("<div class='GooFlow_item" + mark + "' name='" + json.name + "' id='" + id + "'nodeType='" +
					...
					"'" +
					" state='" + json.state + "'>" +
					"<table cellspacing='1' style='width:" + (json.width * this.$scale) + "px;'>" +
					"<tr>" +
					"<td class='ico'><svg class='t-icon' aria-hidden='true'  " +
					"style='width: " + (32 * this.$scale) + "px; height: " + (32 * this.$scale) + "px; " +
					"font-size: " + (32 * this.$scale) + "px; transform: none; margin: 0px '><use xlink:href='#icon-" + json.type +
					"'></use></svg></td>" +
				...
				this.$nodeDom[id] = $("<div class='GooFlow_item" + mark + "' name='" + json.name + "' id='" + id + "'nodeType='" +
					json.type + "' style='top:" + json.top + "px;left:" + json.left + "px' data='" + "' " +
					" state='" + json.state + "'>" +
			...
			this.$nodeDom[id] = $("<div class='GooFlow_item item_round" + mark + "' id='" +
				...
				(json.height * this.$scale) +
				"px;'><tr><td class='ico'><svg class='t-icon' aria-hidden='true'><use xlink:href='#icon-" + json.type +
				"'></use></svg></td></tr></table>" +
		...
		if (this.color.node) {
			if (json.type.indexOf(" mix") > -1) {
				this.$nodeDom[id].css({
					"background-color": this.color.mix,
					"border-color": this.color.mix
				});
			} else {
				this.$nodeDom[id].css({
					"background-color": this.color.node,
					"border-color": this.color.node
				});
			}
			if (mark && this.color.mark) {
				this.$nodeDom[id].css({
					"border-color": this.color.mark
				});
			}
	...
	initWorkForNode: function() {
		...
		this.$workArea.delegate(".ico", "mousedown", {
			...
			X = ev.x - t.left + This.$workArea[0].parentNode.scrollLeft + scrollBarLeft;
			Y = ev.y - t.top + This.$workArea[0].parentNode.scrollTop + scrollBarTop;
			var vX = X - This.$nodeData[id].left * This.$scale;
			var vY = Y - This.$nodeData[id].top * This.$scale;
			var isMove = false;
			// 移动流程节点事件
			document.onmousemove = function(e) {
				if (!e) e = window.event;
				var ev = mousePosition(e);
				if (X == ev.x - vX && Y == ev.y - vY)
					return false;
				X = ev.x - vX;
				Y = ev.y - vY;
				...
				else if (X + This.$workArea[0].parentNode.scrollLeft + This.$nodeData[id].width * This.$scale > t.left + This.$workArea
					.width())
					X = t.left + This.$workArea.width() - This.$workArea[0].parentNode.scrollLeft - This.$nodeData[id].width *
					This.$scale + scrollBarLeft;
				if (Y < t.top - This.$workArea[0].parentNode.scrollTop) {
					Y = t.top - This.$workArea[0].parentNode.scrollTop + scrollBarTop;
				} else if (Y + This.$workArea[0].parentNode.scrollTop + This.$nodeData[id].height * This.$scale > t.top + This
					.$workArea.height()) {
					Y = t.top + This.$workArea.height() - This.$workArea[0].parentNode.scrollTop - This.$nodeData[id].height *
						This.$scale + scrollBarTop;
				}
				...
				X = (X + This.$workArea[0].parentNode.scrollLeft + scrollBarLeft - t.left) / This.$scale;
				Y = (Y + This.$workArea[0].parentNode.scrollTop - t.top + scrollBarTop) / This.$scale;
				// 寻找水平和垂直接近的节点
				var similarData = This.findSimilar(id, X, Y);
				var horiSimiData = similarData[0];
				var vectSimiData = similarData[1];
				if (horiSimiData != null) {
					X = horiSimiData.left;
				}
				if (vectSimiData != null) {
					Y = vectSimiData.top;
				}
				This.moveNode(id, X, Y);
				This.drawAuxiliaryLine(id);
			}
			// 移动流程节点完成
			document.onmouseup = function(e) {
				/*if(isMove) {
					This.moveNode(id, (X + This.$workArea[0].parentNode.scrollLeft + scrollBarLeft - t.left) / This.$scale, 
							(Y + This.$workArea[0].parentNode.scrollTop - t.top + scrollBarTop) / This.$scale);
				}
				This.$ghost.empty().hide();
				document.onmousemove=null;
				document.onmouseup=null;*/
		...
		this.$workArea.delegate(".GooFlow_item", "mouseenter", {
			...
			// 改变背景色
			//			console.log(".GooFlow_item, mouseenter");
			//			console.log($(this).data("new"))
			// 正在增加中的节点不变色
			/*if (typeof($(this).data("new")) != "undefined") {
				return;
			}*/
			$(this).css("background-color", e.data.inthis.color.mark);
		...
		this.$workArea.delegate(".GooFlow_item", "mouseleave", {
			...
			if ($(this).children("div:eq(0)").css("display") != 'block') {
				$(this).css("background-color", e.data.inthis.color.node);
			}
			if (e.data.inthis.$nowType != "direct" && !document.getElementById("GooFlow_tmp_line")) {
		...
		this.$workArea.delegate(".GooFlow_item", "mousedown", {
			...
			var line = e.data.inthis.drawLine("GooFlow_tmp_line", [X, Y], [X, Y], true, true);
			This.$draw.appendChild(line);
		});
		//绑定连线时确定结束点
			...
			if (lineStart && !This.$mpTo.data("p")) {
				...
				if (!This.$nodeData[this.id].marked) {
					...
					if (this.id != This.$focus) {
						$(this).css("border-color", e.data.inthis.color.node);
					} else {
						$(this).css("border-color", e.data.inthis.color.line);
					}
				}
			}
		});
		...
		this.$workArea.delegate(".GooFlow_item > div > div[class!=rs_close]", "mousedown", {
			...
			This.$ghost.css({
				display: "block",
				width: (This.$nodeData[id].width + 6) * This.$scale + "px",
				height: (This.$nodeData[id].height + 6) * This.$scale + "px",
		...
		// 鼠标点击节点图标，点击节点类型图标时，首先创建一个不可见的节点
		this.$tool.on("mousedown", {
			inthis: this
		}, function(e) {
			//			console.log("mouse down " + isMoved);
			if (!e) e = window.event;
			var tar;
			switch (e.target.tagName) {
				case "SPAN":
			...
			document.onmousemove = function(e) {
				...
				let scrollLeft = This.$workArea[0].parentNode.scrollLeft;
				let scrollTop = This.$workArea[0].parentNode.scrollTop;
				...
				var originPosX = newPosX;
				// 移动时将移动中的流程节点中心点置于鼠标指针位置
				newPosX = newPosX < -20 ? -20 : newPosX;
				newPosY = newPosY < 0 ? 0 : newPosY;
				//				console.log(newPosX + "-" + This.$workArea.width() + " scrollLeft " + scrollLeft + " gundongtiao " + (document.getElementById("gundongtiao").scrollLeft))
				//				console.log(This.$nodeDom[tmpID].width());
				// 流程节点不能超出画布右侧
				if (newPosX > (This.$workArea.width() - This.$nodeDom[tmpID].width())) {
					newPosX = This.$workArea.width() - This.$nodeDom[tmpID].width();
				}
				// 流程节点不能超出画布下方边线
				if (newPosY > (This.$workArea.height() - This.$nodeDom[tmpID].height())) {
					newPosY = This.$workArea.height() - This.$nodeDom[tmpID].height();
				}
				// 流程节点按照移动最小单位的长度移动
				newPosX = Math.round(newPosX / initUnit) * initUnit;
				newPosY = Math.round(newPosY / initUnit) * initUnit;
				// 寻找水平和垂直接近的节点
				var similarData = This.findSimilar(tmpID, newPosX, newPosY);
				var horiSimiData = similarData[0];
				var vectSimiData = similarData[1];
				if (horiSimiData != null) {
					newPosX = horiSimiData.left;
				}
				if (vectSimiData != null) {
					newPosY = vectSimiData.top;
				}
				// 设置节点位置
				...
				// 判断并画辅助线
				This.drawAuxiliaryLine(tmpID);
			}
			document.onmouseup = function(e) {
				//				console.log("click up" + isMoved);
				This.$ghost.hide();
				// 如果已经移动节点表明节点添加成功，节点计数器 +1
				if (isMoved) {
					This.$nodeDom[tmpID].css("display", "block");
					This.$max++;
					// This.$nodeDom[tmpID].removeData("new");
				} else {
					// 如果没有移动此节点，删除节点
					This.delNode(tmpID);
				}
	...
	// 查找与目标节点相同位置的节点
	findSimilar(tmpID, newPosX, newPosY) {
		var horiNo = 0,
			vertNo = 0;
		var data, horiData = [],
			vertData = [];
		...
		for (var k1 in this.$nodeData) {
			...
			data = this.$nodeData[k1];
			console.log(newPosX + "-" + newPosY);
			console.log(data.name + "-" + data.left + "-" + data.top);
			if (newPosY >= (data.top - similarPX) && newPosY <= (data.top + similarPX)) {
				vertData[horiNo++] = data;
			}
			if (newPosX >= (data.left - similarPX) && newPosX <= (data.left + similarPX)) {
				horiData[vertNo++] = data;
			}
		}
		if (horiData.length > 0) {
			similarData.push(horiData[0]);
		} else {
			similarData.push(null);
		}
		if (vertData.length > 0) {
			similarData.push(vertData[0]);
		} else {
			similarData.push(null);
		}
		return similarData;
	},
	drawAuxiliaryLine(tmpID) {
		// 判断是否存在与本节点处于同一水平线和同一垂直线的节点, 如果存在则画出水平和垂直线
		var horiNo = 0,
			vertNo = 0;
		var data, horiData = [],
			vertData = [];
		var newPosX = this.$nodeData[tmpID].left;
		var newPosY = this.$nodeData[tmpID].top;
		//		console.log("this.pos: " + newPosX + ", " + newPosY);
		//		console.log("data: ")
		for (var k1 in this.$nodeData) {
			// 过滤掉本节点
			if (k1 == tmpID) {
			...
			//			console.log(data);
			if (data.top == newPosY) {
				horiData[horiNo++] = data;
			}
			if (data.left == newPosX) {
				vertData[vertNo++] = data;
			}
		}
		this.clearAuxiliaryLine();
		//画水平线
		if (horiData.length > 0) {
			var horiDataX = horiData[0].left;
			var horiDataY = horiData[0].top;
			var line = this.drawSampleLine(0, newPosY, this.$workArea.width(), horiDataY);
			this.$draw.appendChild(line);
			this.$auxiliaryLine.push(line);
			horiData = [];
		}
		//画垂直线
		if (vertData.length > 0) {
			var vertDataX = vertData[0].left;
			var vertDataY = vertData[0].top;
			var line = this.drawSampleLine(newPosX, 0, vertDataX, this.$workArea.height());
			this.$draw.appendChild(line);
			this.$auxiliaryLine.push(line);
			vertData = [];
		}
	},
	clearAuxiliaryLine: function() {
		// 删除辅助线
		if (this.$auxiliaryLine.length > 0) {
			for (let x in this.$auxiliaryLine) {
				this.$draw.removeChild(this.$auxiliaryLine[x]);
			}
		}
		this.$auxiliaryLine = [];
	},
	...
	blurItem: function() {
		//		if(this.$focus != ""){
		if (typeof(this.$focus) != "undefined" && this.$focus != "") {
			var jq = $("#" + this.$focus);
			if (jq.prop("tagName") == "DIV") {
				...
				jq.css("background-color", this.color.node);
				if (this.color.line) {
					if (this.$nodeData[this.$focus].marked) {
						jq.css("border-color", this.color.mark);
					} else {
						jq.css("border-color", this.color.node);
					}
				}
			} else {
				...
				if (this.useSVG != "") {
					if (!this.$lineData[this.$focus].marked) {
						//libin 如果线是未选中而且state=true
						if (this.$lineData[this.$focus].state) {
							jq[0].childNodes[1].setAttribute("stroke", this.color.lineSuccess);
							jq[0].childNodes[1].setAttribute("stroke-width", this.brush.width.blur);
							jq[0].childNodes[1].setAttribute("marker-end", "url(#arrow_lineSuccess)");
						} else {
							jq[0].childNodes[1].setAttribute("stroke", this.color.line);
							jq[0].childNodes[1].setAttribute("stroke-width", this.brush.width.blur);
							jq[0].childNodes[1].setAttribute("marker-end", "url(#arrow1)");
						}
					}
				} else {
					if (!this.$lineData[this.$focus].marked) jq[0].strokeColor = this.color.line;
				}
	...
	focusItem: function(id, bool) {
		...
		if (jq.prop("tagName") == "DIV") {
			if (bool && this.onItemFocus != null && !this.onItemFocus(id, "node"))
				return;
			jq.css("background-color", this.color.mark);
			// 增加删除节点的div
			if (editable) jq.children("div:eq(0)").css("display", "block");
			// 不需要向画布区域增加流程节点
			...
			if (this.onItemFocus != null && !this.onItemFocus(id, "line"))
				return;
			if (this.useSVG != "") {
				jq[0].childNodes[1].setAttribute("stroke", this.color.lineMarked);
				jq[0].childNodes[1].setAttribute("stroke-width", this.brush.width.focus);
				jq[0].childNodes[1].setAttribute("marker-end", "url(#arrow2)");
			} else {
				jq[0].strokeColor = this.color.lineMarked;
			}
			if (!editable) return;
			var x, y, from, to, n;
			if (this.useSVG != "") {
	...
	clearDataAndLoadData: function(data) {
		this.clearData();
		this.loadData(data);
	},
	...
	drawSampleLine: function(newPosX, newPosY, horiDataX, horiDataY) {
		//		stroke-dasharray
		var line = document.createElementNS("http://www.w3.org/2000/svg", "line");
		line.setAttribute("x1", newPosX);
		line.setAttribute("y1", newPosY);
		line.setAttribute("x2", horiDataX);
		line.setAttribute("y2", horiDataY);
		line.setAttribute("stroke", "blue");
		line.setAttribute("stroke-width", 0.5);
		line.setAttribute("style", "stroke-dasharray: 5.5");
		return line;
	},
	//绘制一条箭头线，并返回线的DOM
	drawLine: function(id, sp, ep, mark, dash, state) {
		...
		if (this.useSVG != "") {
			...
			//				debugger
			console.log("draw line mark:" + mark + " state: " + state)
			if (mark) { //libin 修改
				/*if(state){
					path.setAttribute("stroke","green");
					path.setAttribute("marker-end","url(#arrow2_1)");
				} else {
					path.setAttribute("stroke", this.color.mark || this.lineColor);
					path.setAttribute("marker-end","url(#arrow2)");
				}*/
				path.setAttribute("stroke-width", 2.0);
				path.setAttribute("stroke", this.color.lineMarked);
				path.setAttribute("marker-end", "url(#arrow2)");
			} else {
				if (state) {
					path.setAttribute("stroke", this.color.lineSuccess);
					path.setAttribute("marker-end", "url(#arrow_lineSuccess)");
				} else {
					path.setAttribute("stroke", this.color.line);
			...
			if (id != "" && id != "GooFlow_tmp_line") {
				...
				text.setAttribute("fill", this.color.font);
				...
				text.style.fontSize = 12 * this.$scale + "px";
			}
		} else {
			...
			if (mark) line.strokeColor = this.color.mark;
			else line.strokeColor = this.color.line;
			line.fillColor = this.color.line;
		}
		var This = this;
	...
	drawPoly: function(id, sp, m1, m2, ep, mark, state) {
		...
		if (this.useSVG != "") {
			...
			poly.style.cursor = "pointer";
			if (mark)
				poly.strokeColor = this.color.mark;
			else
				poly.strokeColor = this.color.line;
		}
		var This = this;
	...
	calcStartEnd: function(n1, n2, scale) {
		...
		var start, end;
		//X判断：
		var x11 = n1.left * scale,
			x12 = n1.left * scale + (n1.width + 6) * scale,
			x21 = n2.left * scale,
			x22 = n2.left * scale + (n2.width + 6) * scale;
		var y11 = n1.top * scale,
			y12 = n1.top * scale + (n1.height + 6) * scale,
			y21 = n2.top * scale,
			y22 = n2.top * scale + (n2.height + 6) * scale;
	...
	calcPolyPoints: function(n1, n2, type, M, scale) {
		...
		var N1 = {
			left: n1.left * scale,
			top: n1.top * scale,
			width: (n1.width + 6) * scale,
			height: (n1.height + 6) * scale
		};
		var N2 = {
			left: n2.left * scale,
			top: n2.top * scale,
			width: (n2.width + 6) * scale,
			height: (n2.height + 6) * scale
		};
		M = M * scale;
		//开始/结束两个结点的中心
		var SP = {
	...
	addLineDom: function(id, lineData) {
		...
		var n1 = this.$nodeData[lineData.from],
			n2 = this.$nodeData[lineData.to];
		if (!n1 || !n2)
			return;
		//开始计算线端点坐标
		var res;
		if (lineData.type && lineData.type != "sl") {
			res = this.calcPolyPoints(n1, n2, lineData.type, lineData.M, this.$scale);
		} else {
			res = this.calcStartEnd(n1, n2, this.$scale);
		}
		if (!res)
			return;
		...
		if (lineData.type == "sl") {
			//libin 添加state
			//			this.$lineDom[id]=GooFlow.prototype.drawLine(id,res.start,res.end,lineData.marked,false,lineData.state);
			this.$lineDom[id] = this.drawLine(id, res.start, res.end, lineData.marked, false, lineData.state);
		} else {
			if (this.$title == "designer") {
				this.$lineDom[id] = this.drawPoly(id, res.start, res.m1, res.m2, res.end, lineData.marked, false, lineData.state);
			} else {
				this.$lineDom[id] = this.drawPoly(id, res.start, res.m1, res.m2, res.end, lineData.marked, lineData.state);
			}
		}
		...
		if (this.useSVG == "") {
			...
			if (lineData.type != "sl") {
				...
				this.$lineDom[id].childNodes[1].style.left = (res.m2[0] + res.m1[0]) / 2 - Min - this.$lineDom[id].childNodes[1].offsetWidth /
					2 + 4;
				Min = (res.start[1] > res.end[1] ? res.end[1] : res.start[1]);
				if (Min > res.m2[1]) Min = res.m2[1];
				if (Min > res.m1[1]) Min = res.m1[1];
				this.$lineDom[id].childNodes[1].style.top = (res.m2[1] + res.m1[1]) / 2 - Min - this.$lineDom[id].childNodes[1].offsetHeight /
					2;
			} else
				this.$lineDom[id].childNodes[1].style.left =
				((res.end[0] - res.start[0]) * (res.end[0] > res.start[0] ? 1 : -1) - this.$lineDom[id].childNodes[1].offsetWidth) /
				2 + 4;
		} else {
		...
		for (var i in this.$lineData) {
			...
			if (this.$lineData[i].from == id) { //找结束点
				...
				if (this.$lineData[i].type == "sl")
					res = this.calcStartEnd(node, other, this.$scale);
				else {
					res = this.calcPolyPoints(node, other, this.$lineData[i].type, this.$lineData[i].M, this.$scale)
				}
				if (!res) break;
				...
				if (other == null) continue;
				if (this.$lineData[i].type == "sl")
					res = this.calcStartEnd(other, node, this.$scale);
				else {
					res = this.calcPolyPoints(other, node, this.$lineData[i].type, this.$lineData[i].M, this.$scale);
				}
				if (!res) break;
			}
			if (other == null) continue;
			...
			if (this.$lineData[i].type == "sl") {
				this.$lineDom[i] = this.drawLine(i, res.start, res.end, this.$lineData[i].marked);
			} else {
				this.$lineDom[i] = this.drawPoly(i, res.start, res.m1, res.m2, res.end, this.$lineData[i].marked);
			}
			...
			if (this.useSVG == "") {
				...
				if (this.$lineData[i].type != "sl") {
					...
					if (Min > res.m2[0]) Min = res.m2[0];
					if (Min > res.m1[0]) Min = res.m1[0];
					this.$lineDom[i].childNodes[1].style.left = (res.m2[0] + res.m1[0]) / 2 - Min - this.$lineDom[i].childNodes[1].offsetWidth /
						2 + 4;
					Min = (res.start[1] > res.end[1] ? res.end[1] : res.start[1]);
					if (Min > res.m2[1]) Min = res.m2[1];
					if (Min > res.m1[1]) Min = res.m1[1];
					this.$lineDom[i].childNodes[1].style.top = (res.m2[1] + res.m1[1]) / 2 - Min - this.$lineDom[i].childNodes[1].offsetHeight /
						2 - 4;
				} else
					this.$lineDom[i].childNodes[1].style.left =
					((res.end[0] - res.start[0]) * (res.end[0] > res.start[0] ? 1 : -1) - this.$lineDom[i].childNodes[1].offsetWidth) /
					2 + 4;
	...
	setLineType: function(id, newType, M) {
		...
		if (newType != "sl") {
			//如果是变成折线
			var res = this.calcPolyPoints(this.$nodeData[from], this.$nodeData[to], this.$lineData[id].type, this.$lineData[id]
				.M, this.$scale);
			if (M) {
			...
			this.$lineMove.hide().removeData("type").removeData("tid");
			res = this.calcStartEnd(this.$nodeData[from], this.$nodeData[to], this.$scale);
			if (!res) return;
			this.$draw.removeChild(this.$lineDom[id]);
			this.$lineDom[id] = this.drawLine(id, res.start, res.end, this.$lineData[id].marked || this.$focus == id);
			this.$draw.appendChild(this.$lineDom[id]);
			if (this.useSVG == "") {
	...
	//设置折线中段的X坐标值（可左右移动时）或Y坐标值（可上下移动时）, 并重新画线
	setLineM: function(id, M, noStack) {
		...
		var ps = this.calcPolyPoints(this.$nodeData[from], this.$nodeData[to], this.$lineData[id].type, this.$lineData[id].M,
			this.$scale);
		this.$draw.removeChild(this.$lineDom[id]);
		// 重新画线
		this.$lineDom[id] = this.drawPoly(id, ps.start, ps.m1, ps.m2, ps.end, this.$lineData[id].marked || this.$focus ==
			id);
		this.$draw.appendChild(this.$lineDom[id]);
		if (this.useSVG == "") {
			this.$lineDom[id].childNodes[1].innerHTML = this.$lineData[id].name;
			...
			this.$lineDom[id].childNodes[1].style.left = (ps.m2[0] + ps.m1[0]) / 2 - Min - this.$lineDom[id].childNodes[1].offsetWidth /
				2 + 4;
			Min = (ps.start[1] > ps.end[1] ? ps.end[1] : ps.start[1]);
			if (Min > ps.m2[1]) Min = ps.m2[1];
			if (Min > ps.m1[1]) Min = ps.m1[1];
			this.$lineDom[id].childNodes[1].style.top = (ps.m2[1] + ps.m1[1]) / 2 - Min - this.$lineDom[id].childNodes[1].offsetHeight /
				2 - 4;
	...
	markItem: function(id, type, mark) {
		if (type == "node") {
			...
			if (this.useSVG != "") {
				if (mark) {
					this.$nodeDom[id].childNodes[1].setAttribute("stroke", this.color.mark);
					this.$nodeDom[id].childNodes[1].setAttribute("marker-end", "url(#arrow2)");
				} else {
					this.$nodeDom[id].childNodes[1].setAttribute("stroke", this.color.line);
					this.$nodeDom[id].childNodes[1].setAttribute("marker-end", "url(#arrow1)");
				}
			} else {
				if (mark) this.$nodeDom[id].strokeColor = this.color.mark;
				else this.$nodeDom[id].strokeColor = this.color.line
			}
		}
		if (this.$undoStatck) {
	...
	addArea: function(id, json) {
		...
		this.$areaDom[id] = $("<div id='" + id + "' class='GooFlow_area area_" + json.color + "' style='top:" + json.top *
			this.$scale + "px;left:" + json.left * this.$scale + "px'><div class='bg' style='width:" + (json.width * this.$scale) -
			2 + "px;height:" + (json.height * this.$scale) - 2 + "px'></div>" +
			"<label>" + json.name +
	...
	exportDiagram: function(fileName) {
		...
		$("body").append(
			'<div id="demo_export" style="position:absolute;top:0;left:0;z-index:-1;width:0px;height:0px;overflow:hidden">' +
			'<div style="color:#15428B;position:absolute;left:0;right:0;width:' + width + 'px;height:' + height +
			'px;overflow:hidden;float:none;" class="GooFlow GooFlow_work"></div>' +
			'</div>');
	...
	print: function(scale) {
		...
		printDiv.children(".GooFlow_work_group").css({
			width: max.width + "px",
			height: max.height + "px"
		});
		printDiv.children("svg").css({
			width: max.width + "px",
			height: max.height + "px"
		});
		printDiv.children(".GooFlow_work_vml").css({
			width: max.width + "px",
			height: max.height + "px"
		});
		printDiv = printDiv.wrap("<div class='GooFlow GooFlow_work'></div>").parent();
		if (this.color.font) {
			printDiv.css("color", this.color.font);
		}
		printDiv.css({
