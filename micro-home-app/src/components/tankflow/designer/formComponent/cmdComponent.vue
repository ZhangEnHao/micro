				...
				<Row>
			     ...
			     <Row>
			       ...
			       <Row>
			        ...
			        <div style="width:100%" v-if="createModel.serPolicyId">
				    	<!--<p slot="title">
				                        服务策略参数
				        </p>-->
				        <Divider />
			       ...
			       <FormItem prop="exceptionRepeat"  v-if="createModel.exceptionCode == 'REPEAT'" label="重复次数">
			         ...
			         <FormItem prop="commandContent" label="命令">
			            <Input v-model="createModel.commandContent" type="textarea" style="width:430px;max-height:400px;overflow-y: auto;" />
			        </FormItem>
			    </Form>
			</Card>
...
<script>
	...
	export default{
		...
		methods: {
			...
			cancelHandler(){
				this.$emit("closeComponent","cmd");
			},
			initServiceStrategy(){
				this.$http.get("strategy/list",{params:{busType:"COMMAND"},root:"WF"}).then(res=>{
