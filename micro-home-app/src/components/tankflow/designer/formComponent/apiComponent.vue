			...
			<Form  ref="createForm" :model="createModel"  :rules="createRules" :label-width="96">
				...
				<Row>
			      ...
			      <Row>
					<Col span="12"> 
			            ...
			            <Select v-model="createModel.serPolicyId" filterable clearable @on-change="serPolicyChange" style="width:200px">
					...
					<Col span="12">
						 <FormItem prop="isAutoNode" label="自动执行" >
				...
				<div v-if="createModel.serPolicyId">
				        ...
				        <template v-for="item in serviceStrategyParamList">
			        		<FormItem  :label="item.paramName">
						        ...
						        <template v-else-if="item.paramTypeCode == 'INPUT'">
							            <Input type="textarea" v-model.trim="item.value" style="width:200px"/>
						        </template>
					...
					<Col span="12"> 
			            ...
			            <Select v-model="createModel.exceptionCode" clearable style="width:200px">
			        ...
			        <FormItem prop="isActivity" label="是否激活" >
			            <Checkbox v-model="createModel.isActivity" true-value="Y" false-value="N"></Checkbox>
			        </FormItem>
			        <div v-if="createModel.isActivity == 'Y'" :bordered="false">
				        <!--<p slot="title">
				                                     激活信息
				        </p>-->
				         <Divider />
				        <FormItem prop="activityTime" label="激活时间">
			        		<TimePicker format="HH:mm"  v-model="createModel.activityTime" style="width:200px"></TimePicker>
				        </FormItem>
				        <Row>
				        	<Col span="12">
				        		<FormItem prop="activityAlarm" label="是否告警">
				            <Checkbox v-model="createModel.activityAlarm" true-value="Y" false-value="N"></Checkbox>
				        </FormItem>
				        	</Col>
				        	<Col span="12">
				        		 <FormItem prop="activityAlarmTime" label="告警时间" v-if="createModel.activityAlarm == 'Y'">
				        	<TimePicker format="HH:mm"  v-model="createModel.activityAlarmTime" style="width:200px"></TimePicker>
				        </FormItem>
				        	</Col>
				        </Row>
				        <Divider />
				    </div>
			        <FormItem prop="isCircle" label="循环执行">
			            <Checkbox v-model="createModel.isCircle" true-value="Y" false-value="N"></Checkbox>
			        </FormItem>
				    <div style="width:100%" v-if="createModel.isCircle == 'Y'">
				    	<Divider />
				       <!-- <p slot="title">
				                                     循环信息
				        </p>-->
				        <FormItem prop="circleInterval"  label="循环间隔(s)">
				            <InputNumber  v-model.trim="createModel.circleInterval" :min="1" :precision="0" style="width:100px"></InputNumber >
				        </FormItem>
				        <FormItem prop="circleNum" label="循环次数">
				            <InputNumber   v-model.trim="createModel.circleNum" :min="0" :precision="0" style="width:100px"></InputNumber >(0无限循环)
				        </FormItem>
				        <Divider />
				    </div>
			    </Form>
...
<script>
	...
	export default{
		...
		methods: {
			...
			cancelHandler(){
				this.$emit("closeComponent","api");
			},
			initServiceStrategy(){
				this.$http.get("strategy/list",{params:{busType:"API"},root:"WF"}).then(res=>{
